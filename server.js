...
// Foydalanuvchi @atlas_ilm kanaliga a'zo ekanligini Telegram Bot API orqali tekshiradi.
// MUHIM: bot shu kanalda ADMIN bo'lishi shart, aks holda getChatMember xato qaytaradi.
app.post('/api/check-subscription', async (req, res) => {
  ...
  if (!BOT_TOKEN) {
    // Token sozlanmagan bo'lsa, foydalanuvchilarni bloklab qo'ymaslik uchun o'tkazib yuboramiz
  ...
  try {
    ...
    const status = tgData.result.status; // 'creator' | 'administrator' | 'member' | 'left' | 'kicked'
    ...
    return res.json({ isSubscribed: true }); // tarmoq xatosida foydalanuvchini bloklamaymiz
  }
});
